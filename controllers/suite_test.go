...
var (
	reconciler   *DockerBuildReconciler
...
func runInContext(registerControllers func(*testenv.Environment), run func() error, crdPath string) error {
	...
	utilruntime.Must(buildv1alpha1.AddToScheme(scheme.Scheme))
...
func TestMain(m *testing.M) {
	...
	runInContext(func(testEnv *testenv.Environment) {
		controllerName := "dockerBuild-controller"
		...
		if err := (reconciler).SetupWithManager(testEnv); err != nil {
			panic(fmt.Sprintf("Failed to start DockerBuildReconciler: %v", err))
		}
	}, func() error {
		code = m.Run()
		return nil
...
func applyGitRepository(objKey client.ObjectKey, artifactName string, revision string) error {
	...
	opt := []client.PatchOption{
		...
		client.FieldOwner("dockerBuild-controller"),
	}
